// ==ClosureCompiler==
// @output_file_name bear.js
// @compilation_level SIMPLE_OPTIMIZATIONS
// @code_url http://ajax.googleapis.com/ajax/libs/jquery/1.3.2/jquery.js
// @code_url http://beardemo.kumasystem.com/bear/jquery.cookie.js
// ==/ClosureCompiler==
...
( function($) {
	...
	$.fn.bearAjaxForm = function(options) {
		$(this).each(
			function(key, value){
				var options = $.extend( {}, $.bear.ajaxDefaults, options);
				var loading;
				try {
				...
				$(this + "input[type='submit']").bind(options.event, options, $.bear.ajaxEvent);
	  ...
	  ajaxReq : function(options) {
		...
		$.ajax( {
			...
			beforeSend : function(XMLHttpRequest) {
				...
				// BEAR default session key is _s
				if($.cookie('_s')){
					XMLHttpRequest.setRequestHeader('X-Bear-Session-Verify', $.cookie('_s'));
				}
			...
			success : function(json) {
				if (options.success) {
					// app success
					try {
						$.app[options.success](XMLHttpRequest);
					} catch(e){
						$.$.bear.log(e);
					}
				}
